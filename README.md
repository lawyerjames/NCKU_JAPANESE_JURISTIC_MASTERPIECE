...
- 🔊 **互動式日文語音朗讀 (Web Speech Audio Player)**：
  - **純淨文本抽取技術**：自動在音訊引擎處理前移除 `<rt>` 假名標籤與 `(0)`、`(1)` 聲調數字，避免語音引擎將漢字與假名重複朗讀。
  - **浮動播放控制面板**：支援全文連續朗讀/暫停/停止、`0.75x`~`1.5x` 語速動態調節，以及單段卡片「朗讀本段」按鈕。
  - **自動聚焦高亮**：朗讀時當前段落會自動以靛藍光環高亮並平滑捲動置中，方便隨聽隨讀。
